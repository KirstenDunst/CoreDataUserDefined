...
- (void)deleteDataWithTableName:(NSString *)tableName WithPredicate:(NSPredicate *)predicate{
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription *entity = [NSEntityDescription entityForName:tableName inManagedObjectContext:self.managedObjectContext];
    [fetchRequest setEntity:entity];
    ...
    NSArray *fetchedObjects = [self.managedObjectContext executeFetchRequest:fetchRequest error:&error];
    if (fetchedObjects) {
        for (People *p in fetchedObjects) {
            //删除
...
- (NSArray *)selectFromDataWithPredicate:(NSPredicate *)predicate{
    
    //1.获取请求对象,需要把表名加入到请求对象
